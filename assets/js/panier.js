...
arrayPrd.map((Prd) => {
  ...
  fetch(`http://localhost:3000/produits/${prd[0]}`)
    ...
    .then((product) => {
      ...
      pProduct.innerHTML = `<div id="product-details" class="flex items-center justify-between flex-wrap">
                            <div class="flex  gap-5">
                                ...
                                <div class="w-72 flex flex-col justify-around">
                                    ...
                                    <h2 class="font-semibold text-xl text-red-600"><span id="unit-price">${
                                      product.price
                                    }</span> $</h2>
                                </div>
                            </div>
                            <div class="flex" data-id="${prd[1]}">
                                ...
                                <input id="qtt-display" class="outline-none font-medium text-lg py-2 bg-slate-200 text-center w-14" type="number" value="${
                                  prd[1]
                                }" min="1" max="${product.stock}">
                                <button id="inc-qtt-btn" class="font-medium text-lg px-4 py-2 rounded-r-sm bg-gray-200" type="button">+</button>
                            </div>
                            <h1 class="font-medium text-xl"><span id="total-price" >${
                              product.price * prd[1]
                            }</span> $</h1>
      ...
      document
        .getElementById("panier-products-container")
        .appendChild(pProduct);

      // Mettre à Jour le Nombre des Produits Ajoutés au Panier
      document.getElementById("cout-products").innerText =
        document.getElementById("panier-products-container").childElementCount;
      ...
      tpProduct.innerHTML = `<h1>${product.name}</h1>
        <h2 class="font-medium mx-5">x <span id="${product.id}">${
        prd[1]
      }</span></h2>
        <h3 id="total-${product.id}" class="font-semibold justify-self-end">${
        product.price * prd[1]
      }</h3>`;
      ...
      pProduct
        ...
        .addEventListener("click", function () {
          ...
          currentTargetPrice.innerText =
            (currentTarget.value * currentPrice).toFixed(2) + " $";

          // Affichage du Quantité et du Prix Totale du Produit Séléctionné
          document.getElementById(`${product.id}`).innerText =
            currentTarget.value;
          const variantIncPrice = document.getElementById(
            `total-${product.id}`
          );
          variantIncPrice.innerText = (
            currentTarget.value * currentPrice
          ).toFixed(2);
      ...
      pProduct
        ...
        .addEventListener("click", function () {
          ...
          varDecPrice.innerText = (currentTarget.value * currentPrice).toFixed(
            2
          );
          ...
          confirmDelete.onclick = function () {
            ...
            document.getElementById("cout-products").innerText =
              document.getElementById(
                "panier-products-container"
              ).childElementCount;
          };

          if (
            document.getElementById("panier-products-container")
              .childElementCount == "0"
          ) {
            document
              .getElementById("panier-message")
